...
public class Player {
	...
	public Player(final int id, final String name, final int groupe, final int sexe, final int classe, final int color1,
			...
			final long prison, final boolean isNew, final String parcho, final long timeDeblo, final boolean noall, String succes) {
	...
	public VipMaitre get_Maitre()
	{
		return (this._maitre);
	}
	
	public void set_Maitre(VipMaitre newMaitre)
	{
		this._maitre = newMaitre;
	}
	...
	public void addSucces(final int succes) {
		if (this.staticEmote.containsKey(succes)) {
			return;
		}
		this.staticEmote.put(succes, succes);
		if (!this.isOnline()) {
			return;
		}
		SocketManager.GAME_SEND_EMOTE_LIST(this, getCompiledEmote(this.getStaticEmote(), this.getDynamicEmote()));
		SocketManager.GAME_SEND_STATS_PACKET(this);
		SocketManager.send(this, "eA" + succes);
	}
