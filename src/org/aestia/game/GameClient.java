...
public class GameClient {
	...
	public void gameJoinFight(final String packet) {
		if (this.perso.get_fight() != null) {
			return;
		}
		if (this.perso.isDead() == 1) {
	...
	private void readyFight(final String packet) {
		...
		{
			if (perso.get_Maitre().getMaitre() == perso)
			for (Player p: perso.get_Maitre().getpersos())
			{
				if (p == null)
					continue ;
				if (!p.isOnline())
					continue ;
				if (p.getCurMap() != perso.getCurMap())
					continue ;
				if (p.getGameClient() == null)
					continue ;
				p.getGameClient().readyFightEsclave(packet);
			}
		}
	...
	private void readyFightEsclave(String packet)
	{
		if (perso.get_fight() == null)
			return;
		if (perso.get_fight().getState() != Constant.FIGHT_STATE_PLACE)
			return;
		perso.set_ready(packet.substring(2).equalsIgnoreCase("1"));
		perso.get_fight().verifIfAllReady();
		SocketManager.GAME_SEND_FIGHT_PLAYER_READY_TO_FIGHT(perso.get_fight(), 3, perso.getId(), packet.substring(2).equalsIgnoreCase("1"));
	}
