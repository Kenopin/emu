...
public class CommandPlayer {
	...
	static {
	 ...
	 public static boolean command(String msg, String command) {
	...
	public static boolean analyse(final Player perso, final String msg) {
		...
		if(msg.length() > 7 && msg.substring(1, 7).equalsIgnoreCase("parcho")) {
			...
			{
				perso.set_Maitre(null);
				
					SocketManager.GAME_SEND_MESSAGE(perso, "Mode Maitre desactivé.");
					
				
				return true;
			}
				ArrayList<Player> group = new ArrayList<Player>();
				for (Player p : perso.getCurMap().getPersos())
				{
					if (p == null)
						continue;
					if (p == perso)
					...
					if (!p.isOnline())
						continue;
					if (p.getAccount().getCurIP().compareTo(perso.getAccount().getCurIP()) == 0)
						if (p != perso)
						{
							group.add(p);
							//String pathstr = Pathfinding.getShortestStringPathBetween(perso.getCurMap(), p.getCurCell().getId(), perso.getCurCell().getId(), 0);
							//p.getGameClient().newactiondeplacement(pathstr);
						}
				}
				if (group.size() == 0)
					return true;
				perso.set_Maitre(new VipMaitre(perso, group, true));
				
					SocketManager.GAME_SEND_MESSAGE(perso, "Mode Maitre activé.");
	    	...
	    	short map = perso.getCurMap().getId();
	    	int cell = perso.getCurCell().getId();
	    	 if (perso.isInPrison())
	             return false;
	         if (perso.cantTP())
	             return false;
	    	...
	    	if (perso.get_Maitre() != null)
			{
				...
				for (Player pe : perso.get_Maitre().getpersos())
				{
					pe.teleport(map, cell);
				}
